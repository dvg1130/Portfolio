...
func (c *SDKClient) SDKRegister(register models.Credentials) (*models.RegisterResponse, error) {
	// marshal creds into JSON
	bodyBytes, err := json.Marshal(register)
	if err != nil {
		return nil, fmt.Errorf("failed to marshal credentials: %w", err)
	}

	// build the request
	url := "http://127.0.0.1:8080/api/sdk/register"
